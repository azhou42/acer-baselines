...
def test(rank, variant, T, shared_model):
  torch.manual_seed(variant['seed'] + rank)

  env = gym.make(variant['env'])
  env.seed(variant['seed'] + rank)
  model = ActorCritic(env.observation_space, env.action_space, variant['hidden_size'])
  model.eval()

  save_dir = os.path.join('results', variant['name'])  

  can_test = True  # Test flag
  t_start = 1  # Test step counter to check against global counter
  rewards, steps = [], []  # Rewards and steps for plotting
  l = str(len(str(variant['T-max'])))  # Max num. of digits for logging steps
  done = True  # Start new episode

  # stores step, reward, avg_steps and time 
  results_dict = {'t': [], 'reward': [], 'avg_steps': [], 'time': []}

  while T.value() <= variant['T-max']:
    if can_test:
      t_start = T.value()  # Reset counter

      # Evaluate over several episodes and average results
      avg_rewards, avg_episode_lengths = [], []
      for _ in range(variant['evaluation-episodes']):
        while True:
          # Reset or pass on hidden state
          if done:
            # Sync with shared model every episode
            model.load_state_dict(shared_model.state_dict())
            hx = torch.zeros(1, variant['hidden_size'])
            cx = torch.zeros(1, variant['hidden_size'])
          ...
          reward_sum += reward
          done = done or episode_length >= variant['max-episode-length']  # Stop episodes at a max length
      ...
      print(('[{}] Step: {:<' + l + '} Avg. Reward: {:<8} Avg. Episode Length: {:<8}').format(
            ...
            t_start,
            sum(avg_rewards) / variant['evaluation-episodes'],
            sum(avg_episode_lengths) / variant['evaluation-episodes']))
      fields = [t_start, sum(avg_rewards) / variant['evaluation-episodes'], sum(avg_episode_lengths) / variant['evaluation-episodes'], str(datetime.now())]

      # storing data in the dictionary.
      results_dict['t'].append(t_start)
      results_dict['reward'].append(sum(avg_rewards) / variant['evaluation-episodes'])
      results_dict['avg_steps'].append(sum(avg_episode_lengths) / variant['evaluation-episodes'])
      ...
      if T.value() - t_start >= variant['evaluation-interval']:
        can_test = True
